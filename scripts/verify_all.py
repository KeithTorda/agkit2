...
VERIFICATION_SUITE = (
    ...
    CheckSpec("Test Suite", "skills/testing-patterns/scripts/test_runner.py", "P3 Testing", required=True),
    ...
    CheckSpec("GEO Check", "skills/seo-fundamentals/scripts/geo_checker.py", "P5 SEO & Content"),
    ...
    CheckSpec("Lighthouse Audit", "skills/performance-profiling/scripts/lighthouse_audit.py", "P8 Runtime", target="url", timeout=180),
    CheckSpec("Playwright Smoke Test", "skills/testing-patterns/scripts/playwright_runner.py", "P8 Runtime", target="url", timeout=120),
)
...
def main() -> int:
    ...
    for spec in VERIFICATION_SUITE:
        if args.no_runtime and spec.target == "url":
            continue
        if args.no_e2e and spec.name == "Playwright Smoke Test":
            continue
        specs.append(spec)
