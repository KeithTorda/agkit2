...
WHAT'S YOUR STATE COMPLEXITY?
        ...
        │   │
        │   └── Zustand or Jotai for shared client state (useState for local;
        │       │   React Context only for rarely-changing values such as theme or session)
        │       ├── Minimal boilerplate
        │       ├── Easy to understand
        │       └── Scales OK to medium
        │
        ...
        │   │
        │   └── Zustand with slices + TanStack Query
        │       ├── Predictable, debuggable (devtools middleware)
        │       ├── Redux Toolkit + RTK Query only in existing code
...
|--------------|--------------|-----------------|
| **Redux for simple app** | Massive overkill | Zustand or Jotai; React Context only for rarely-changing values |
...
> These trees guide thinking, not rules to follow blindly. Every project has unique constraints; choose for actual needs, and ask when requirements are vague (questions: the global `core-protocol` rule).
