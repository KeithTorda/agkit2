...
def detect_project_type(project_path: Path) -> dict:
    ...
    if (project_path / "composer.json").exists():
        ...
        if (vendor_bin / "pint").exists() or (vendor_bin / "pint.bat").exists():
            result["linters"].append({"name": "pint", "cmd": ["php", "vendor/bin/pint", "--test"]})
        else:
            result.setdefault("notes", []).append("Laravel Pint not installed - `composer require laravel/pint --dev` recommended")
        if (vendor_bin / "phpstan").exists() or (vendor_bin / "phpstan.bat").exists():
            result["linters"].append({"name": "phpstan", "cmd": ["php", "vendor/bin/phpstan", "analyse", "--no-progress", "--memory-limit=1G"]})
        elif (project_path / "artisan").exists():
            result.setdefault("notes", []).append("Larastan not installed - `composer require larastan/larastan --dev` recommended")

    # Python project
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        result["type"] = "python"
        
        # Only run tools that are actually installed; a missing linter is reported, not failed.
        if shutil.which("ruff"):
            result["linters"].append({"name": "ruff", "cmd": ["ruff", "check", "."]})
        else:
            result.setdefault("notes", []).append("ruff not installed - `pip install ruff` recommended")
        pyproject = project_path / "pyproject.toml"
        mypy_configured = (project_path / "mypy.ini").exists() or (
            pyproject.exists() and "[tool.mypy]" in pyproject.read_text(encoding="utf-8", errors="replace")
        )
        if shutil.which("mypy") and mypy_configured:
            result["linters"].append({"name": "mypy", "cmd": ["mypy", "."]})
        elif shutil.which("pyright"):
            result["linters"].append({"name": "pyright", "cmd": ["pyright"]})
