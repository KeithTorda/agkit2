...
Minimal correct tool definition (object root, described properties, explicit `required`, bounded inputs, `additionalProperties: false`, typed output):

```json
{
  "name": "search_orders",
  "description": "Search a customer's orders by email. Read-only.",
  "inputSchema": {
    "type": "object",
    "properties": {
      "email":  { "type": "string",  "format": "email", "description": "Customer email to match exactly" },
      "status": { "type": "string",  "enum": ["open", "shipped", "cancelled"], "description": "Optional status filter" },
      "limit":  { "type": "integer", "minimum": 1, "maximum": 100, "default": 20, "description": "Max orders to return" }
    },
    "required": ["email"],
