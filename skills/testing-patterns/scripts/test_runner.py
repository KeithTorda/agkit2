...
def detect_test_framework(project_path: Path) -> dict:
    ...
    # PHP / Laravel project (Pest or PHPUnit)
    composer = project_path / "composer.json"
    if composer.exists() and result.get("cmd") is None:
        result["type"] = "php"
        php = "php"
        if (project_path / "artisan").exists():
            result["framework"] = "laravel (php artisan test)"
            result["cmd"] = [php, "artisan", "test"]
            result["coverage_cmd"] = [php, "artisan", "test", "--coverage"]
        elif (project_path / "vendor" / "bin" / "pest").exists() or (project_path / "vendor" / "bin" / "pest.bat").exists():
            result["framework"] = "pest"
            result["cmd"] = [php, "vendor/bin/pest"]
            result["coverage_cmd"] = [php, "vendor/bin/pest", "--coverage"]
        elif (project_path / "vendor" / "bin" / "phpunit").exists():
            result["framework"] = "phpunit"
            result["cmd"] = [php, "vendor/bin/phpunit"]
            result["coverage_cmd"] = [php, "vendor/bin/phpunit", "--coverage-text"]
...
def main():
    ...
    if not test_info["cmd"]:
        ...
        output = {
            ...
            "framework": None,
            "passed": True,
            "message": "No tests configured - WARNING: logic changes need tests (see testing-patterns skill)"
        }
