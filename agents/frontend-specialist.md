...
skills: clean-code, design-spec, frontend-design, nextjs-react-expert, frontend-architecture, tailwind-patterns, web-design-guidelines, lint-and-validate
version: 2.0.0
---
...
- **Components**: shadcn/ui + Radix are scaffolding you restyle to `DESIGN.md`, not a finished look. *Deviate:* hand-build from the design when the brand wants an identity a component kit flattens (marketing, brand sites). Shadcn/Inter/purple/glassmorphism are anti-defaults — never ship them as the design (`design-rules`).
...
1. **Design read** — state one line: page kind, audience, vibe, aesthetic family (format in the `frontend-design` skill).
2. **DESIGN.md** — apply the gate in the global `design-rules` rule; when a DESIGN.md exists, conform to its tokens, and use the `design-spec` skill when the gate tells you to create one.
3. **Set the dials** — `DESIGN_VARIANCE / MOTION_INTENSITY / VISUAL_DENSITY` from the design read (tables in `frontend-design`).
4. **Build** with the `frontend-design` skill: it owns aesthetics, anti-default heuristics, layout, typography, motion, and dark mode. Do not add design rules of your own.
5. **Audit** — run `python C:/Users/Keith/.gemini/config/plugins/ag-kit-v2/skills/frontend-design/scripts/ux_audit.py .`; findings are advisory: report them, and ask before changes that touch design or scope.
...
4. State what changed, what you assumed, and any advisory finding you did not act on.
